...
class Dataset:
    ...
    def parse_to_word_list(self, parse_col_name=None, stop=False, alphanum_sep=False, blind_num=False):
        if stop:     
            self.__set_all([[clear_stop_words(word_tokenize(split_alphanum(x, alphanum_sep, blind_num).lower())) for x in df[parse_col_name]] for df in self.get_all()], parse_col_name)
        else:
            self.__set_all([[word_tokenize(split_alphanum(x, alphanum_sep, blind_num).lower()) for x in df[parse_col_name]] for df in self.get_all()], parse_col_name)
...
def split_alphanum(text, flag=True, blind_numbers=False):
    if flag:
